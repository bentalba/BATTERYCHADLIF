...
import { ArrowLeft, Grid3X3, List, SlidersHorizontal, X, Truck } from "lucide-react";
...
const BatteriesPoidsLourd = () => {
  const { toast } = useToast();
  const categoryProducts = useMemo(() => getProductsByCategory('poids-lourd'), []);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Header with orange accent for PRO */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-bold">Batteries Poids Lourds</h1>
                  <span className="px-2 py-0.5 bg-white/20 rounded-full text-xs font-semibold">PRO</span>
                </div>
                <p className="text-white/80 text-sm">Camions, bus, véhicules agricoles</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {hasActiveFilters && (
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {activeFilters.brands.map(brand => (
                <span key={brand} className="inline-flex items-center gap-1 px-2 py-1 bg-orange-100 text-orange-800 rounded-full text-xs">
                  {brand}
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          >
            ...
            <div className="sticky bottom-0 bg-white border-t p-4">
              <Button onClick={() => setShowMobileFilters(false)} className="w-full bg-orange-500 hover:bg-orange-600">
...
export default BatteriesPoidsLourd;
